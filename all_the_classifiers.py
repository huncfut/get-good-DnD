...
data = pd.read_csv("data/dndData.csv").head(100000)

stats = ["s_str", "s_dex", "s_con", "s_int", "s_wis", "s_cha"]

init_X_train, init_X_test, y_train, y_test = train_test_split(data[stats], data["class_starting"], random_state=2137, test_size=0.2)

preprocessing = StandardScaler().fit(init_X_train)

X_train = preprocessing.transform(init_X_train)
X_test = preprocessing.transform(init_X_test)
